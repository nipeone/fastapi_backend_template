...
from backend.app.services.service_casbin import casbin_service
...
async def get_pagination_casbin(
    ptype: Annotated[Optional[str], Query(description='策略类型, p / g')] = None,
    sub: Annotated[Optional[str], Query(description='用户 uuid / 角色')] = None,
) -> ResponseModel:
    page_data = await casbin_service.get_pagination(ptype=ptype, sub=sub)
