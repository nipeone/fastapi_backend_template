...
async def create_access_token(sub: str, expires_delta: Optional[timedelta] = None, **kwargs) -> tuple[str, datetime]:
    """
...
async def create_refresh_token(sub: str, expire_time: Optional[datetime] = None, **kwargs) -> tuple[str, datetime]:
    """
...
async def jwt_authentication(token: str) -> User:
    """
    JWT authentication，在jwt_decode的基础上验证token的有效时间
    ...
    async with async_session() as db:
        user = await get_current_user_with_relation(db, user_id)
    if not user:
        raise TokenError(msg='token 无效')
    if not user.status:
...
async def get_current_user_with_relation(db: AsyncSession, user_id: int) -> User:
    """
